...
if __name__ == '__main__':
    ...
    for s in config.TRAIN.S_RANGE:
        for m in config.TRAIN.M_RANGE:
            file_name = '[s,m Basis+Relative] ' + f'LAYERS_NUM:{config.BEST.NUM_LAYER} '+ f'DIM:{config.BEST.OUT_CHANNELS} ' + f'ACT:{config.BEST.ACT} ' + f's:{s} ' + f'm:{m}'
            #
            writer = SummaryWriter(log_dir=f'./tb_logger/find_sm/{file_name}')
            #
            fc_metric = ArcFace(num_layer=config.BEST.NUM_LAYER, in_features=4, out_features=config.BEST.OUT_CHANNELS, num_class=NUM_JOINTS+NUM_TOKEN,
                                use_embedding=config.BEST.USE_EMB, activation=config.BEST.ACT, s=s,
            ...
            save_dir = f"/home/jysuh/PycharmProjects/coord_embedding/checkpoint/find_sm/{file_name}"
            ...
            #
            nn_embedding_save_path = os.path.join(save_dir, "nn_embedding_model.pt")
