...
class Video_Loader(Dataset):
    def __init__(self, config, data_path):
        ...
        self.workout_vocab, self.condition_vocab = self.get_vocab()
        ...
        for exercise_name in tqdm(self.data.keys(), total=len(list(self.data.keys())), desc='preprocess'):
            for view_idx  in self.data[exercise_name].keys():
                for video_idx in self.data[exercise_name][view_idx].keys():
                    ...
                    is_sorted = keys[1:] == sorted(keys[1:], key=lambda x: int(x))
                    if not is_sorted:
                        raise ValueError(
    ...
    def __getitem__(self, idx):
        ...
        exercise_name = self.workout_vocab[self.videos[idx][1]]
        view_idx = self.videos[idx][2]
        conditions = self.videos[idx][3]
        # tmp_conditions = torch.zeros(len(list(self.condition_vocab.keys())))
        return video, exercise_name, view_idx, conditions

    def collate_fn(self, batch):
        videos, exercise_name, view_idx, conditions = zip(*batch)
        return videos, exercise_name, view_idx, conditions
