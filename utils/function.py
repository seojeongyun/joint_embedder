...
def plot_tsne_with_centroids(config, feats, labels, vocab):
    try:
        import torch
    ...
    # ---------- (C) Visualization ----------
        ...
        if s_vals is not None:
            ...
            png_path = os.path.join(out_dir, f"{config.FILE_NAME}.png")
...
#     if C > 1 and len(Xo_eval) >= 10 and len(Xo_eval) > C:
#         try:
#             s_vals = silhouette_samples(Xo_eval, yo,
#                                         metric=config.VIS.PLOT_METRIC_METHOD)  # silhouette value for each sample s(i)
#             sil_overall = float(np.mean(s_vals))  # overall mean silhouette score
#             # compute mean silhouette for each cluster
#             sil_per_class = {
#                 int(c): float(np.mean(s_vals[yo == c])) for c in np.unique(yo) if np.sum(yo == c) > 1
