...
if __name__ == '__main__':
    ...
    token_idx = torch.LongTensor(token_idx).to(device)

    #
    title = 'train_numlayer_b_b+r_dim_s_m'
            ...
            for DIM in config.TRAIN.EMB_DIM:
                for s in config.TRAIN.S_RANGE:
                    for m in config.TRAIN.M_RANGE:
                        if USE_EMB:
                            file_name = '[B+R] ' + f'LAYERS_NUM:{num_layers} ' + f'DIM:{DIM} ' + 'GELU ' + f's:{s} ' + f'm:{m}'
                        else:
                            file_name = '[R] ' + f'LAYERS_NUM:{num_layers} ' + f'DIM:{DIM} ' + 'GELU ' + f's:{s} ' + f'm:{m}'
                        #
                        print(file_name)
                        writer = SummaryWriter(log_dir=f'./tb_logger/{title}/{file_name}')
                        #
                        fc_metric = ArcFace(num_layer=num_layers, in_features=4, out_features=DIM, num_class=NUM_JOINTS+NUM_TOKEN,
                                            use_embedding=USE_EMB, activation='GELU', s=s,
                                            m=m, device=device).to(device)
                        fc_metric.train()
                        #
                        ...
                        for epoch in range(config.TRAIN.EPOCH):
                            ...
                            for i, (J_coord, J_tokens, WRKOUT, FRAME, VIEW, VIDEO) in enumerate(train_loader):
                                ...
                                #
                                J_tokens = torch.cat((token_idx, J_tokens[0].to(device)))
                        ...
                        save_dir = f"/home/jysuh/PycharmProjects/coord_embedding/checkpoint/{title}/{file_name}"
                        ...
                        #
                        nn_embedding_save_path = os.path.join(save_dir, "nn_embedding.pth.tar")
